...
namespace Mediapipe.Unity
{
  public class StaticImageSource : ImageSource
  {
    ...
    [SerializeField] private RawImage[] _availableSources;
    ...
    private Texture image
    {
      get
      {
        ...
        {
          image = _availableSources[0].texture;
        }
        return _image;
      }
      set
    ...
    public override void SelectSource(int sourceId)
    {
      ...
      image = _availableSources[sourceId].texture;
    }
    ...
    {
      // // get the current version of the camera texture
      // // InitializeOutputTexture(_availableSources[0].texture);
      // Debug.Log("imagesource- GetCurrentTexture:  " + Time.time);
      // Texture2D _texture = _availableSources[0].texture as Texture2D;
      // return _texture;
      ...
      Destroy(_outputTexture); // to free memory
      InitializeOutputTexture(_availableSources[0].texture);
      return _outputTexture;
    ...
    //private void InitializeOutputTexture(RawImage src_RawImage)
    private void InitializeOutputTexture(Texture src)
    {
      Debug.Log("imagesource- InitializeOutputTexture: " + Time.time);
      //Texture src = src_RawImage.texture;
